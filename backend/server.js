...
const PUMPFUN_PREPARED_DB_PATH = IS_VERCEL_RUNTIME ? path.join("/tmp", "getmeajob-pumpfun-prepared.json") : path.join(ROOT, "cache", "pumpfun-prepared.json");
const PUMPFUN_SESSIONS_DB_PATH = IS_VERCEL_RUNTIME ? path.join("/tmp", "getmeajob-pumpfun-sessions.json") : path.join(ROOT, "cache", "pumpfun-sessions.json");
...
app.use((error, _req, res, next) => {
  if (error?.type === "entity.too.large") {
    return res.status(413).json({ error: "Selected evidence file is too large to upload through Get Me a Job." });
  }
  return next(error);
});
...
function normalizeAlphaTokenAddress(value = "", chainId = 1) {
  const text = sanitizeAlphaText(value || "", 500);
  if (!text) return "";
  if (isSolanaAlphaChain(chainId)) {
    const solanaAddress = normalizeSolanaAddress(text);
...
function normalizeAlphaWalletAddress(value = "", chainId = 1) {
...
function normalizeAlphaTip(row = {}) {
  ...
  const author = normalizeAlphaWalletAddress(row.author || row.address || "", chainId) || ethers.ZeroAddress;
  const authorWallet = normalizeAlphaWalletAddress(row.authorWallet || row.tipWallet || author || "", chainId) || ethers.ZeroAddress;
...
function defaultJobTipsAlphaTip() {
  return normalizeAlphaTip({
    id: "job-tip-interview-follow-up",
    title: "Send the follow-up within 24 hours",
    projectName: "Interview follow-up",
    tokenSymbol: "JOBS",
    tokenAddress: "So11111111111111111111111111111111111111112",
    chainId: 101,
    minBalance: "0",
    category: "Interview, Resume, Freelance",
    confidence: "high",
    teaser: "After an interview, send a short note that proves you listened and restates the exact value you can deliver.",
    body:
      "Use three parts: thank them for the specific conversation, mention one problem they said they need solved, and close with one concrete next step. Keep it under 120 words. For freelance work, use the same format after a client call and include the deliverable you can start first.",
    evidenceUrl: "",
    evidenceType: "",
    author: ethers.ZeroAddress,
    authorWallet: ethers.ZeroAddress,
    authorName: "Get Me a Job",
    xHandle: "getmeajob",
    xName: "Get Me a Job",
...
function applyAlphaSystemTips(store = {}) {
  ...
  const hasJobTipSeed = tips.some((tip) => {
    ...
    return tip.id === "job-tip-interview-follow-up" || title.includes("follow-up within 24 hours") || handle === "getmeajob";
  });
  if (!hasJobTipSeed) {
    const seeded = defaultJobTipsAlphaTip();
...
function pumpFunPreparedId() {
  return `gmj-${Date.now().toString(36)}-${crypto.randomBytes(5).toString("hex")}`;
}
...
function emptyGoStore() {
  ...
  return {
    bounties: [
      {
        id: "go-landing-page",
        title: "Build a one-page portfolio site",
        description: "Design and build a clean responsive portfolio landing page for a job seeker.",
        deliverables: ["Live preview URL", "Source files or repository", "Mobile responsive layout"],
        rewardUsd: 206.92,
        tokenSymbol: "Get Me a Job",
        tokenAmount: 3,
        tokenUnit: "ETH",
        creator: "0xEF1F5aa00C169B2F5ca4f4ab47350e7DB17c84D3",
        creatorName: "Get Me a Job",
        status: "open",
      ...
      {
        id: "go-stream-clip",
        title: "Edit my resume for a frontend role",
        description: "Rewrite a resume for a frontend developer application with clearer bullets and stronger project outcomes.",
        deliverables: ["Edited resume document", "Summary of changes", "Three role-specific bullet options"],
        rewardUsd: 689.45,
        tokenSymbol: "GETMEAJOB",
        tokenAmount: 10,
        tokenUnit: "MON",
      ...
      {
        id: "go-x-account",
        title: "Create five Fiverr gig thumbnails",
        description: "Create five polished gig thumbnails for a freelance services profile.",
        deliverables: ["Five PNG thumbnails", "Editable source file", "Short usage note"],
        rewardUsd: 41.37,
        tokenSymbol: "GETMEAJOB",
        tokenAmount: 0.6,
        tokenUnit: "ETH",
        creator: "0x73230F236c6929192659fd6e8f527303A61325f2",
        creatorName: "Freelance client",
        status: "open",
        imageUri: "",
        createdAt: now - 17 * 60,
...
function normalizeGoBounty(row = {}) {
  ...
  return {
    ...
    tokenSymbol: sanitizeGoText(row.tokenSymbol || "Get Me a Job", 24).replace(/^\$/, "").toUpperCase(),
...
function officialAirdropConfig() {
  const rawToken = String(
    process.env.AIRDROP_TOKEN_ADDRESS ||
      ...
      process.env.GETMEAJOB_AIRDROP_TOKEN ||
  ...
  const chainId = parseChainId(
    process.env.AIRDROP_CHAIN_ID ||
      ...
      process.env.GETMEAJOB_AIRDROP_CHAIN_ID ||
  ...
  return {
    ...
    quoteMode,
    name: String(process.env.AIRDROP_TOKEN_NAME || process.env.OFFICIAL_AIRDROP_TOKEN_NAME || "Get Me a Job").trim(),
    symbol: String(process.env.AIRDROP_TOKEN_SYMBOL || process.env.OFFICIAL_AIRDROP_TOKEN_SYMBOL || "Get Me a Job").trim().replace(/^\$/, "").toUpperCase(),
    message: String(
      process.env.AIRDROP_MESSAGE ||
        "After the official Get Me a Job token launches, creator rewards will be routed back to top holders from this page."
    ).trim()
  };
}
...
function normalizeGoSubmission(row = {}) {
  ...
  return {
    ...
    agentId: normalizeAgentId(row.agentId || ""),
    agentName: sanitizeGoText(row.agentName || "", 80),
    body,
    mediaUrl: sanitizeGoUrl(row.mediaUrl || ""),
    links,
    likes: [...new Set(likes.map((address) => address.toLowerCase()))],
...
function decorateGoBounty(bounty, store = readGoDb()) {
  ...
  return {
    ...bounty,
    submissions: submissions + Math.max(0, Number(bounty?.sourceSubmissionCount || 0) || 0),
...
async function fetchPumpFunBounties(options = {}) {
  ...
  try {
    const response = await withTimeout(
      fetch(sourceUrl, {
        headers: {
          "User-Agent": "GetMeAJobBot/1.0 (+https://getmeajob.fun)",
...
async function draftAgentBountyWithOpenAI(agent = {}, bounty = {}) {
  ...
  const prompt = [
    "You are a Get Me a Job bounty-solving agent.",
...
async function executeAgentBountyWithOpenAI(agent = {}, bounty = {}) {
  ...
  const model = String(process.env.OPENAI_AGENT_MODEL || process.env.OPENAI_MODEL || "gpt-4.1-mini").trim();
  const prompt = [
    "You are a Get Me a Job autonomous bounty agent running inside the Get Me a Job app.",
...
async function generateOpenAiBountyConceptImage(agent = {}, bounty = {}) {
  ...
  const prompt = [
    ...
    `Agent: ${sanitizeAlphaText(agent.name || "Get Me a Job Agent", 80)}`
  ].filter(Boolean).join("\n\n");
  const response = await withTimeout(
  ...
  return {
    filename: `getmeajob-task-scene-${Date.now().toString(36)}.png`,
...
function xCallbackUrl(req) {
  ...
  const strictConfiguredCallback = String(process.env.X_CALLBACK_STRICT || "0") === "1";
  ...
  if (configured) {
    try {
      ...
      if (strictConfiguredCallback || configuredHost === requestHost) return configured;
      console.warn(`Ignoring X_CALLBACK_URL host mismatch: configured=${configuredHost} request=${requestHost}`);
    } catch {
      console.warn("Ignoring invalid X_CALLBACK_URL");
    }
  }
  return `${origin}/api/x/oauth/callback`;
}
...
app.post("/api/pumpfun/kol-buy", async (req, res) => {
  try {
    ...
    if (!kolApplication?.enabled || Number(kolApplication.buySol || 0) <= 0) {
      return res.status(400).json({ error: "KOL buy is not enabled" });
    }
    const kolWallet = new SolanaPublicKey(kolApplication.wallet);
    const rpcUrl = String(process.env.SOLANA_RPC_URL || process.env.PUMPFUN_SOLANA_RPC_URL || CHAIN_META[101].rpcUrls[0]).trim();
    const connection = new SolanaConnection(rpcUrl, "confirmed");
    const [latest, globalInfo] = await Promise.all([
      connection.getLatestBlockhash("confirmed"),
      connection.getAccountInfo(PUMP_SDK_MODULE.GLOBAL_PDA, "confirmed")
    ]);
    if (!globalInfo) {
    ...
    res.status(500).json({ error: error.message || "Unable to build KOL buy transaction" });
  }
});
...
app.post("/api/pumpfun/kol-transfer", async (req, res) => {
  try {
    ...
    const user = new SolanaPublicKey(String(req.body?.userPublicKey || "").trim());
    const kolApplication = sanitizeKolApplication(req.body?.kolApplication);
    if (!kolApplication?.enabled) return res.status(400).json({ error: "KOL transfer is not enabled" });
    const kolWallet = new SolanaPublicKey(kolApplication.wallet);
    ...
    if (tokenAmount <= 0n) return res.status(400).json({ error: "KOL transfer token amount is missing" });
    ...
    res.status(500).json({ error: error.message || "Unable to build KOL transfer transaction" });
  }
});
  ...
  try {
    ...
    if (!official.configured) {
      return res.status(400).json({
        error: "Official airdrop token is not configured yet. Launch the Get Me a Job token, then set AIRDROP_TOKEN_ADDRESS and AIRDROP_CHAIN_ID in Vercel."
      });
    }
    ...
    if (!launch) {
      return res.status(404).json({ error: "Token was not found in this Get Me a Job factory" });
    }
...
app.post("/api/upload-file", async (req, res) => {
  try {
    ...
    const filepath = path.join(UPLOADS_DIR, filename);
    fs.writeFileSync(filepath, binary);
      res.json({ url: `/uploads/${filename}`, publicUrl: `${getPublicBaseUrl(req)}/uploads/${filename}`, mime });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      note: externalSubmitRequired
        ? `${execution.note}${generatedAttachmentNote} Prepared for Pump.fun. Open the original bounty and run the Get Me a Job page assistant.`
...
app.post("/api/go/bounties/:id/submissions", async (req, res) => {
  try {
    const id = normalizeGoId(req.params.id || "", "go");
    const store = readGoDb();
    const bounty = (store.bounties || []).find((row) => normalizeGoId(row?.id || "", "go") === id) || await findLiveGoBounty(id);
...
app.post("/api/alpha", async (req, res) => {
  try {
    ...
    const author = normalizeAlphaWalletAddress(body.author || body.address || "", chainId);
    if (!author) throw new Error("author wallet is required");
    const authorWallet = normalizeAlphaWalletAddress(body.authorWallet || body.tipWallet || "", chainId);
    if (!authorWallet) throw new Error(isSolanaAlphaChain(chainId) ? "valid SOL tip wallet is required" : "tip wallet is required");
