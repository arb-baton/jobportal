...
function updatePreview() {
  const typedName = ui.name.value.trim() || "";
  const name = typedName || "Your Job Title";
  const symbol = FIXED_JOB_TOKEN_SYMBOL;
  if (ui.symbol) ui.symbol.value = FIXED_JOB_TOKEN_SYMBOL;
  const description = ui.description.value.trim() || "Your resume appears here.";
  ...
  const src = explicitImage || makeFallbackImage(typedName, symbol);
...
function setupFormEnhancements() {
  ...
  ui.symbol.addEventListener("input", () => { ui.symbol.value = FIXED_JOB_TOKEN_SYMBOL; onInput(); });
  ...
  if (ui.devBuyEth) ui.devBuyEth.value = DEFAULT_PUMPFUN_STARTER_BUY_SOL;
...
function setSubmitting(active, label = "") {
  ...
  ui.launchSubmitBtn.textContent = active ? label || "Launching..." : "Launch Job Application";
}
  ...
  ui.launchResultList.innerHTML = rows
    .map((row) => {
      ...
      const href = ok
        ...
        : "#";
      const body = ok
        ? `<a href="${href}">Open ${escapeHtml(label)} job ${escapeHtml(shortAddress(row.token))}</a>`
...
async function prepareLaunchDetails() {
  ...
  const symbol = FIXED_JOB_TOKEN_SYMBOL;
  if (ui.symbol) ui.symbol.value = FIXED_JOB_TOKEN_SYMBOL;
  const totalSupplyInput = ui.supply?.value?.trim?.() || "1000000000";
  ...
  if (!imageUri) {
    imageUri = isPumpFunMode() ? `${window.location.origin}/assets/pump-r-logo.png` : makeFallbackImage(name, symbol);
    if (ui.image) ui.image.value = imageUri;
  }
...
async function launchPumpFun(details) {
  ...
  setAlert(ui.alert, "Open Phantom to sign first. Get Me a Job will add the mint signature after your approval, simulate the transaction, then broadcast through the configured Solana RPC.");
...
async function onCreate(event) {
  ...
  try {
    setSubmitting(true, "Launching job application...");
    renderLaunchResults([]);
    if (ui.resultLink) {
...
async function init() {
  ...
  await loadChainConfig(101).catch(() => loadChainConfig(getPreferredChainId() || 1));
  ...
  window.addEventListener("GetMeAJob:chainChanged", handleChainChanged);
