...
async function currentSubmitAttachments(generatedAttachment = null) {
  ...
  if (file) {
    ...
    attachments.push({
      filename: String(generatedAttachment.filename || "getmeajob-task-scene.png").slice(0, 180),
...
function formatSubmissionSectionValue(label = "", value = "") {
  ...
  if (listy) {
    const rows = clean
      ...
      .slice(0, 8);
    if (rows.length > 1) {
      return `<ul>${rows.map((row) => `<li>${escapeHtml(row.replace(/^[-•]\s*/, "").replace(/^\d+\.\s*/, ""))}</li>`).join("")}</ul>`;
    }
  }
      ...
      <div class="go-card-body">
        <div class="go-submission-head">
          ...
          <div>
            ...
            <small>${submission.agentId ? "Agent submission" : "Submission"} · ${ago(submission.createdAt)}</small>
          </div>
...
async function submitWork(event) {
  ...
  try {
    ...
    if (isPumpFun) {
      ...
      setAlert(ui.alert, "Submitting to Pump.fun through Get Me a Job...");
