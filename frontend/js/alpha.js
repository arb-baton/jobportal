    ...
    ? hot
        .map((tip, index) => `
          <button class="alpha-hot-row" type="button" data-scroll-alpha="${escapeHtml(tip.id)}">
            <span>${index + 1}</span>
            <b>${escapeHtml(tip.category || "Tip")}</b>
            <small>${escapeHtml(tip.projectName || tip.title || "Topic")} - ${compact(tip.upvotes || 0)} up</small>
          </button>
        `)
        .join("")
  ...
  return `
    <article class="alpha-card unlocked alpha-pro-card" id="alpha-${escapeHtml(tip.id)}">
      <div class="alpha-card-topline">
        ...
        <span class="alpha-confidence ${escapeHtml(tip.confidence || "medium")}">${escapeHtml(tip.confidence || "medium")} signal</span>
      </div>
      <div class="alpha-card-head">
        <div class="alpha-card-title-wrap">
          <span class="alpha-kicker">${escapeHtml(tip.category || "Job tip")}</span>
          <h2>${escapeHtml(tip.title)}</h2>
          <p>${escapeHtml(tip.teaser || "Practical job tip.")}</p>
        </div>
      ...
      <div class="alpha-meta-grid">
        <div class="alpha-token-row">
          ...
          <div>
            ...
            <small>${escapeHtml(tip.category || "Career advice")}</small>
          </div>
        </div>
...
function renderFeed() {
  ...
  ui.feed.innerHTML = tips.length
    ...
    : `<div class="panel-card alpha-empty"><h3>No tips yet</h3><p>Connect X and submit the first job tip for this feed.</p></div>`;
  renderStats();
}
...
async function submitAlpha(event) {
  ...
  if (evidenceFile && !evidenceUrl) {
    ...
    setAlert(ui.alert, "Uploading job tip evidence...");
  ...
  await api.createAlphaTip({
    ...
    evidenceUrl,
    evidenceType,
    category: ui.category.value || "Job tip",
