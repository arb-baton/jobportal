...
function jobTitleForLaunch(launch = {}) {
  return trimText(String(launch?.name || "Job Application"), 22);
}
...
function chainMetaForLaunch(launch) {
  if (isPumpFunLaunch(launch)) return { chainId: 101, shortName: jobTitleForLaunch(launch), name: "Pump.fun" };
  const chainId = Number(launch?.chainId || state.chainId || 1);
...
function chainClassForLaunch(launch) {
  if (isPumpFunLaunch(launch)) return "job";
  const chainId = Number(launch?.chainId || state.chainId || 1);
  if (chainId === 8453) return "base";
  if (chainId === 143) return "monad";
  if (chainId === 101) return "job";
...
function renderTrending() {
  if (HOME_FEED_CLEARED) {
    if (ui.trendingWrap) ui.trendingWrap.innerHTML = "";
    return;
  }
...
function renderExplore() {
  if (HOME_FEED_CLEARED) {
    if (ui.launchCountLabel) ui.launchCountLabel.textContent = "0 job applications";
    if (ui.launchesWrap) ui.launchesWrap.innerHTML = "";
    return;
  }
  const items = filteredLaunches();
  if (ui.launchCountLabel) {
    ui.launchCountLabel.textContent = `${items.length} ${items.length === 1 ? "job application" : "job applications"}`;
  }
  if (!items.length) {
    const message =
...
async function refreshLaunches(options = {}) {
  ...
  if (!freshLaunches.length) {
    ...
    if (cached.length) {
      ...
      renderTrending();
      renderExplore();
      setAlert(ui.alert, "Live feed is syncing, showing recent cached job applications.");
      return;
    }
  }
...
async function init() {
  ...
  try {
    ...
    if (cached.length) {
      ...
      ui.trendingWrap.innerHTML = `<article class="panel-card"><p class="muted">Unable to load trending job applications right now.</p></article>`;
      ui.launchesWrap.innerHTML = `<article class="panel-card"><p class="muted">Unable to load job applications right now.</p></article>`;
    }
    setAlert(ui.alert, parseUiError(err), true);
  }
