(() => {
  ...
  sideNav?.querySelectorAll('a[href="/onboard"], a[href="/airdrop"], a[href="/communities"], a[href*="trade.padre.gg"]')?.forEach((node) => node.remove());
  sideNav?.querySelectorAll('.side-link-label')?.forEach((node) => {
    if (node.textContent.trim() === 'GO') node.textContent = 'Freelance';
    if (node.textContent.trim() === 'Alpha') node.textContent = 'Job Tips';
  });
  const brandName = sidebar.querySelector('.logo-name');
  if (brandName) brandName.textContent = 'Get Me a Job';
  sidebar.querySelectorAll('.logo-sep, .logo-tld').forEach((node) => node.remove());
